...
$("#tabNewFlightLink").on("shown.bs.tab", function () {
  aNewFlight();
});
...
$("#frmGeneral").submit(function (e) {
  ...
  var divisionLogo = $("#txtDivisionLogo").val();
  ...
  var discordWebhookUrl = $("#txtDiscordWebhookUrl").val();
  var autoTurnover = $("#chkAutoTurnover").is(":checked") ? 1 : 0;
  var timeOnlyInList = $("#chkTimeOnlyInList").is(":checked") ? 1 : 0;
  var dateStart = $("#dtpEventStart")
    .datetimepicker("viewDate")
    .format("YYYY-MM-DD HH:mm:00");
  var dateEnd = $("#dtpEventEnd")
    .datetimepicker("viewDate")
    .format("YYYY-MM-DD HH:mm:00");
  ...
  $.ajax({
    ...
    data: {
      ...
      date_end: dateEnd,
      auto_turnover: autoTurnover,
      time_only_in_list: timeOnlyInList,
    },
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        content +=
          '<tr style="cursor: pointer" onclick="aGetAirport(' +
          data[i].id +
          ')" id="airport' +
          data[i].id +
          '">';
        ...
        if (data[i].airport)
          content +=
            "<td>" + data[i].airport.countryFlag24 + data[i].icao + "</td>";
        else content += "<td>" + data[i].icao + "</td>";
...
function aGetUsers() {
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        content +=
          '<tr style="cursor: pointer" onclick="aGetUser(' +
          data[i].id +
          ')" id="user' +
          data[i].id +
          '">';
        ...
        content +=
          "<td>" +
          (data[i].email.length < 1 ? "(not set)" : data[i].email) +
          "</td>";
    ...
    success: function (data) {
      ...
      if (data.flights.length > 0) {
        $.each(data.flights, function () {
          content +=
            '<a href="javascript:void(0)" class="list-group-item list-group-item-action" onclick="getFlight(' +
            this.id +
            ')">';
          content +=
            (this.airline ? this.airline.logo : "") +
            "<strong>" +
            this.callsign +
            "</strong> " +
            this.originIcao +
        ...
        content +=
          '<a href="javascript:void(0)" class="list-group-item list-group-item-action" style="font-style: italic; text-align: center">(no booked flights yet)</a>';
      content += "</div>";
      ...
      if (data.slots.length > 0) {
        $.each(data.slots, function () {
          content +=
            '<a href="javascript:void(0)" class="list-group-item list-group-item-action" onclick="getSlot(' +
            this.id +
            ')">';
          content +=
            (this.airline ? this.airline.logo : "") +
        ...
        content +=
          '<a href="javascript:void(0)" class="list-group-item list-group-item-action" style="font-style: italic; text-align: center">(no private slots yet)</a>';
      content += "</div>";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        content +=
          '<tr style="cursor: pointer" onclick="aGetTimeframe(' +
          data[i].id +
          ')" id="slot' +
          data[i].id +
          '">';
        ...
        if ((apt = data[i].eventAirport))
          content +=
            "<td>" +
            (apt.airport ? apt.airport.countryFlag24 : "") +
            apt.icao +
            ' <small class="text-muted">' +
            apt.name +
            "</small></td>";
        else content += "<td>" + data[i].airportIcao + "</td>";

        content += "<td>" + data[i].timeHuman + "</td>";
        content += "<td>" + data[i].typeHuman + "</td>";
        content += "<td>";
        content +=
          '<span class="badge badge-success">Free: ' +
          data[i].statistics.free +
          '</span> <span class="badge badge-danger"> Granted: ' +
          data[i].statistics.granted +
          "</span> ";
        if (data[i].statistics.requested > 0)
          content +=
            '<span class="badge badge-warning">Pending: ' +
            data[i].statistics.requested +
            "</span>";
        content += "</td>";
        content += "</tr>";
      }
      ...
      if (pending > 0) {
        if (pending == 1)
          $("#slotAlert").html(
            "You have <strong>one</strong> request waiting for evaluation!"
          );
        else
          $("#slotAlert").html(
            "You have <strong>" +
              pending +
              "</strong> requests waiting for evaluation!"
          );
        $("#slotAlert").show();
      } else $("#slotAlert").hide();
...
$("#numTfNewHourFrom").on("change", function () {
  ...
  if (to <= from) $("#numTfNewHourTo").val(from);
});
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.eventAirport && data.eventAirport.airport)
        $("#timeframeTitle").html(
          "Private slots at " +
            data.timeHuman +
            " at " +
            data.eventAirport.airport.countryFlag24 +
            data.eventAirport.icao
        );
      else
        $("#timeframeTitle").html(
          "Private slots at " + data.timeHuman + " at " + data.aircraftIcao
        );

      if (data.eventAirport)
        $("#txtTfEditAirport").val(
          data.airportIcao + " - " + data.eventAirport.name
        );
      else $("#txtTfEditAirport").val(data.airportIcao);

      $("#dtpTfEditDate").datetimepicker("date", moment(data.time));
      $("#selTfEditType").val(Number(data.type));
      ...
      if (data.slots) {
        for (var i = 0; i < data.slots.length; i++) {
          ...
          if ((airline = row.airline))
            tbl +=
              ...
              row.callsign +
              "</td>";
          else tbl += "<td>" + row.callsign + "</td>";

          tbl +=
            '<td data-toggle="tooltip" title="' +
            (row.aircraftName ? row.aircraftName : "") +
            '">' +
            row.aircraftIcao +
            (row.aircraftFreighter ? "/F" : "") +
            "</td>";
          ...
          if ((dest = row.destinationAirport))
            tbl +=
              ...
              dest.icao +
              "</span></td>";
          else tbl += "<td>" + row.destinationIcao + "</td>";

          tbl += "<td>" + row.bookedAtHuman + "</td>";

          if (row.booked == "requested")
            tbl +=
              '<td><button class="btn btn-warning btn-sm btn-block" onclick="getSlot(' +
              row.id +
              ')"><i class="far fa-hand-paper"></i> Requested by <strong>' +
              row.bookedBy +
              "</strong></button></td>";
          else
            tbl +=
              '<td><button class="btn btn-danger btn-sm btn-block" onclick="getSlot(' +
              row.id +
              ')"><i class="far fa-handshake"></i> Granted to <strong>' +
              row.bookedBy +
              "</strong></button></td>";
          tbl += "</tr>";
        }
        ...
        tbl +=
          '<tr><td colspan="6" style="font-style: italic; text-align: center">(no slots yet)</td></tr>';
...
$("#frmTimeframeEdit").submit(function (e) {
  ...
  var type = $("#selTfEditType").val();
  var time = $("#dtpTfEditDate")
    .datetimepicker("viewDate")
    .format("YYYY-MM-DD HH:mm:00");
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && data.error == 0) {
            ...
            toast({
              title:
                "The confirmation emails have been sent with one or more errors.",
              type: "warning",
            });
          } else notification(data);
        },
        ...
        if (result.value) {
          $.ajax({
            ...
            data: {
              ...
              subject: subject,
              message: message,
              recipients_code: recipientsCode,
            },
            success: function (data) {
              if (data && data.error == 0) {
                toast({
